...
export function Summary({ summary }: Summary) {
  const fromDate = dayjs().startOf('week').format('D[ de ]MMM')
  const toDate = dayjs().endOf('week').format('D[ de ]MMM')

  // evita divisão por zero e verifica os valores antes de calcular o percentual
  const completedPercentage =
    summary.total > 0 ? Math.round((summary.completed * 100) / summary.total) : 0
  ...
  return (
    <main className="max-w-[540px] py-10 px-5 mx-auto flex flex-col gap-6">
      ...
      <div className="space-y-6">
        ...
        {/* verifica se goalsPerDay existe antes de iterar */}
        {summary.goalsPerDay &&
